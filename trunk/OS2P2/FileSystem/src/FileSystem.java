...
public class FileSystem {
   ...
   //fileName = Seagate :-)
   //changes to any of these 3 values will automatically reflect to the entire program
   ...
   public static void main(String[] args) throws IOException {
      ...
      //creating a disk object, which will be used later to access the read, write, sync methods
      ...
      while (true) {
         ...
         if (readInputValue == 1) {
            ...
            if (blockNum >= FileSystem.nblocks) {
               //Checking if the entered value exceeds the available number of blocks
               ...
               try {//accepting the data entered by the user
                  ...
                  //copyBuffer function is used to make the 'buffer' byte array size blockSize agian
                  //if this is not done, the byte array being sent to 'writeBlock'
                  //will be of the size of the number of characters, entered at the CLI
                  //this will be a problem since accepting such an input will not help in
                  //overwriting the contents of the disk of a specific block
                  buffer = copyBuffer(buffer);
                  //sending the blockNum and buffer to writeBlock method available in 'disk' object
            ...
            //Exitting the program - instead of 4 the user can actually enter any integer > 3
            System.out.println("Data from cache will be written to disk now.\n"
                    + "Exiting Virtual File System...");
            disk.syncDisk();//performing sync in order to increase efficiency of the disk
            //doing so, will make sure that the contents of the cache are written to disk
   ...
   public static byte[] copyBuffer(byte[] buffer) {//this method is used to make the size of the input data same as
